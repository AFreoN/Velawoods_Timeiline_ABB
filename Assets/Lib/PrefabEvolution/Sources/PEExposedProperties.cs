				...
				if (this.PrefabScript.ParentPrefab != null)
				{
					...
					foreach (var property in InheritedProperties.OfType<ExposedProperty>())
					{
						...
						if (property.Target == null)
							UnityEngine.Debug.Log("Inherited property Error: Local target is not found Path:" + property.PropertyPath, this.PrefabScript);
					}
				}
			}
			return InheritedProperties;
	...
	public class ExposedProperty : BaseExposedData
	{
		...
		public object Value
		{
			get
			{
				if (!IsValid)
				{
					UnityEngine.Debug.LogWarning("Trying to get value from invalid prefab property. Target:" + this.Target + " Property path:" + this.PropertyPath);
					return null;
				}
			...
			set
			{
				if (!IsValid)
				{
					UnityEngine.Debug.LogWarning("Trying to set value to invalid prefab property. [Target:" + this.Target + ", Property path:" + this.PropertyPath + "]");
					return;
				}
