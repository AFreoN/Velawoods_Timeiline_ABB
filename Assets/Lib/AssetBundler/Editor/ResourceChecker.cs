...
public class TextureDetails
{
	...
	public List<Object> FoundInImages=new List<Object>();
...
public class ResourceChecker : EditorWindow {
	...
	[MenuItem ("VELA/Optimisation/Resources/Resource Checker")]
	static void Init ()
	{  
    ...
    bool IsTexturePowerOfTwo(uint x)
    {
        return (x != 0) && ((x & (x - 1)) == 0);
    }
	...
	void ListTextures()
	{
		...
		{			
            ...
            if (tDetails.texture.width == tDetails.texture.height && IsTexturePowerOfTwo((uint)tDetails.texture.height))
            {
            ...
            if (GUILayout.Button((tDetails.FoundInRenderers.Count + tDetails.FoundInImages.Count) + " GO", GUILayout.Width(50)))
            {
                List<Object> FoundObjects = new List<Object>();
                foreach (Renderer renderer in tDetails.FoundInRenderers) FoundObjects.Add(renderer.gameObject);
                foreach (Image image in tDetails.FoundInImages) FoundObjects.Add(image.gameObject);
	...
	void CheckResources()
	{
		...
		// HUG: find all the UI textures
		Image[] uiImages = (Image[]) FindObjectsOfType(typeof(Image));
		foreach(Image uiImage in uiImages) {
			if(uiImage != null) {
				Texture tex = uiImage.mainTexture;
				if(tex != null) {
					var texDetail = GetTextureDetail(tex, null, null, uiImage);
					if(!ActiveTextures.Contains(texDetail)) ActiveTextures.Add(texDetail);
				}
			}
		}
	...
	private TextureDetails GetTextureDetail(Texture tTexture, Material tMaterial=null, MaterialDetails tMaterialDetails=null, Image image=null)
	{
		...
		if(image != null) {
			tTextureDetails.FoundInImages.Add(image);
		}
